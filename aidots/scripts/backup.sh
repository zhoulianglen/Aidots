...
# ─────────────────────────────────────────────
# backup.sh — Back up AI coding tool configs
...
if [[ "$LOCALE" == "zh" ]]; then
    MSG_INIT_PROMPT="首次使用 aidots，请设置备份目录："
    MSG_INIT_OPT1="  [1] 使用默认位置 ~/dotai（推荐）"
    MSG_INIT_OPT2="  [2] 自定义路径"
    MSG_INIT_CHOOSE="请选择 [1/2]: "
    MSG_INIT_CUSTOM="请输入备份目录路径: "
    MSG_CONFIG_SAVED="✅ 配置已保存到 ~/.aidots/config.json"
    MSG_README_GENERATED_BY="由 [aidots](https://github.com/zhoulianglen/aidots) 自动生成于 %s"
    MSG_README_TOOLS_HEADING="备份的工具"
    MSG_README_TH_TOOL="工具"
    MSG_README_TH_FILE_COUNT="配置文件数"
    MSG_README_TH_FILE="文件"
    MSG_README_TH_SIZE="大小"
    MSG_NO_CHANGES="  ℹ️  没有变更需要提交"
    MSG_COMMITTED="✅ 已提交：backup: %s"
    MSG_PUSHED="🚀 已推送到远程仓库"
    MSG_BACKUP_HEADER="📦 aidots 备份"
    ...
    MSG_CONFIG_SAVED="✅ Config saved to ~/.aidots/config.json"
    ...
    MSG_NO_CHANGES="  ℹ️  No changes to commit"
    MSG_COMMITTED="✅ Committed: backup: %s"
    MSG_PUSHED="🚀 Pushed to remote"
    MSG_BACKUP_HEADER="📦 aidots Backup"
...
generate_readme() {
...

    {
        ...
        ' | while IFS= read -r tid; do
            ...
            # Get files for this tool — we need to compute human sizes in bash
...
main() {
    ...
    if [[ -z "$BACKUP_DIR" ]]; then
        if ! BACKUP_DIR=$(read_config); then
            # First-time setup — capture only the last line (the dir path)
            local setup_output
